(function(){"use strict";try{var e=document.createElement("style");e.appendChild(document.createTextNode(".audio-recorder{background-color:#ebebeb;box-shadow:0 2px 5px #bebebe;border-radius:20px;box-sizing:border-box;width:40px;display:flex;align-items:center;padding:12px;transition:all .2s ease-in}.audio-recorder-mic{cursor:pointer;height:16px;color:#000}.audio-recorder-timer,.audio-recorder-status{margin-left:10px;font-family:Segoe UI,Tahoma,Geneva,Verdana,sans-serif;font-size:14px;font-weight:400;line-height:1}.audio-recorder-status{margin-left:15px;display:flex;align-items:baseline;flex-grow:1;animation-name:fading-ar-status;animation-duration:2s;animation-iteration-count:infinite}.audio-recorder-status-dot{background-color:#d00;border-radius:50%;height:10px;width:9px;margin-right:5px}.audio-recorder-options{height:16px;margin-left:10px;cursor:pointer}.recording{border-radius:12px;width:300px;transition:all .2s ease-out}.display-none{display:none}@keyframes fading-ar-status{0%{opacity:1}50%{opacity:0}to{opacity:1}}")),document.head.appendChild(e)}catch(i){console.error("vite-plugin-css-injected-by-js",i)}})();
import j, { useState as a, useCallback as y, useEffect as A } from "react";
const l = () => {
  const [r, N] = a(!1), [u, M] = a(!1), [S, s] = a(0), [t, d] = a(), [i, o] = a(), [z, C] = a(), w = () => {
    const c = setInterval(() => {
      s((e) => e + 1);
    }, 1e3);
    o(c);
  }, D = () => {
    i != null && clearInterval(i), o(void 0);
  };
  return {
    startRecording: y(
      (c) => {
        i == null && (c ? (console.log(c), navigator.mediaDevices.getUserMedia({
          audio: { deviceId: c ? { exact: c } : void 0 }
        }).then((e) => {
          N(!0);
          const g = new MediaRecorder(e);
          d(g), g.start(), w(), g.addEventListener("dataavailable", (I) => {
            C(I.data), g.stream.getTracks().forEach((L) => L.stop()), d(null);
          });
        }).catch((e) => console.log(e))) : navigator.mediaDevices.getUserMedia({ audio: !0 }).then((e) => {
          N(!0);
          const g = new MediaRecorder(e);
          d(g), g.start(), w(), g.addEventListener("dataavailable", (I) => {
            C(I.data), g.stream.getTracks().forEach((L) => L.stop()), d(null);
          });
        }).catch((e) => console.log(e)));
      },
      [i]
    ),
    stopRecording: () => {
      t == null || t.stop(), D(), s(0), N(!1), M(!1);
    },
    togglePauseResume: () => {
      u ? (M(!1), t == null || t.resume(), w()) : (M(!0), D(), t == null || t.pause());
    },
    recordingBlob: z,
    isRecording: r,
    isPaused: u,
    recordingTime: S
  };
}, x = "data:image/svg+xml;base64,", v = "data:image/svg+xml;base64,", Y = "data:image/svg+xml;base64,", b = "data:image/svg+xml;base64,", k = "data:image/svg+xml;base64,";
const Q = ({
  onRecordingComplete: r,
  recorderControls: N,
  deviceId: u,
  classes: M
}) => {
  var T, m, c, e, g, I;
  const {
    startRecording: S,
    stopRecording: s,
    togglePauseResume: t,
    recordingBlob: d,
    isRecording: i,
    isPaused: o,
    recordingTime: z
  } = N != null ? N : l(), [C, w] = a(!1), D = (L = !0) => {
    w(L), s();
  }, n = (L) => {
    S(L);
  };
  return A(() => {
    (C || N) && d != null && r != null && r(d);
  }, [d]), /* @__PURE__ */ j.createElement("div", {
    className: `audio-recorder ${i ? "recording" : ""} ${(T = M == null ? void 0 : M.AudioRecorderClass) != null ? T : ""}`,
    "data-testid": "audio_recorder"
  }, /* @__PURE__ */ j.createElement("img", {
    src: i ? b : x,
    className: `audio-recorder-mic ${(m = M == null ? void 0 : M.AudioRecorderStartSaveClass) != null ? m : ""}`,
    onClick: i ? () => D() : () => n(u),
    "data-testid": "ar_mic",
    title: i ? "Save recording" : "Start recording"
  }), /* @__PURE__ */ j.createElement("span", {
    className: `audio-recorder-timer ${i ? "" : "display-none"} ${(c = M == null ? void 0 : M.AudioRecorderTimerClass) != null ? c : ""}`,
    "data-testid": "ar_timer"
  }, Math.floor(z / 60), ":", String(z % 60).padStart(2, "0")), /* @__PURE__ */ j.createElement("span", {
    className: `audio-recorder-status ${i ? "" : "display-none"} ${(e = M == null ? void 0 : M.AudioRecorderStatusClass) != null ? e : ""}`
  }, /* @__PURE__ */ j.createElement("span", {
    className: "audio-recorder-status-dot"
  }), "Recording"), /* @__PURE__ */ j.createElement("img", {
    src: o ? Y : v,
    className: `audio-recorder-options ${i ? "" : "display-none"} ${(g = M == null ? void 0 : M.AudioRecorderPauseResumeClass) != null ? g : ""}`,
    onClick: t,
    title: o ? "Resume recording" : "Pause recording",
    "data-testid": "ar_pause"
  }), /* @__PURE__ */ j.createElement("img", {
    src: k,
    className: `audio-recorder-options ${i ? "" : "display-none"} ${(I = M == null ? void 0 : M.AudioRecorderDiscardClass) != null ? I : ""}`,
    onClick: () => D(!1),
    title: "Discard Recording",
    "data-testid": "ar_cancel"
  }));
};
export {
  Q as AudioRecorder,
  l as useAudioRecorder
};
